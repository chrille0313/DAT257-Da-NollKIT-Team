...
function createFilterSlice(filterName: string) {
  return createSlice({
    ...
    reducers: {
      setFilter: (state, action: PayloadAction<DropdownOption[]>) => action.payload,
    },
  });
}
...
export const filterReducers = Object.fromEntries(
  Object.entries(filterSlices).map(([filterName, slice]) => [filterName, slice.reducer])
) as Record<RecipeFilterName, ReturnType<typeof createFilterSlice>['reducer']>;

export const filterActions = Object.fromEntries(
  Object.entries(filterSlices).map(([filterName, slice]) => [filterName, slice.actions])
